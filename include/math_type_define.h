...
#define EIGEN_MAKE_TYPEDEFS(Type, TypeSuffix, Size, SizeSuffix)   \
  ...
  typedef Matrix<Type, 1, Size>    RowVector##SizeSuffix##TypeSuffix;
	...
	EIGEN_MAKE_TYPEDEFS(rScalar, d, 9, 9)
...
namespace DyrosMath
{
	...
	const static Eigen::Matrix3d rotationCubicCostume(double time,
		double time_0,
		double time_f,
		const Eigen::Matrix3d &rotation_0,
		const Eigen::Matrix3d &rotation_f)
	...
	const static Eigen::Vector3d rotationCubicDotCostume(double time,
		double time_0,
		double time_f,
		const Eigen::Matrix3d &rotation_0, const Eigen::Matrix3d &rotation_cubic,
		const Eigen::Matrix3d &rotation_f, double hz_)
	{
		...
		double theta_dot = cubicDot(time, time_0, time_f, 0, theta, 0, 0, hz_);
		Eigen::Vector3d w;
